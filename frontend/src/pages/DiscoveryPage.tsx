...
import { discoverMCPServers, getRateLimitStatus } from '../store/slices/researchSlice';
...
export const DiscoveryPage: React.FC = () => {
  ...
  const { servers, isLoading, error, rateLimit } = useSelector((state: RootState) => state.research);
      ...
      <Card className="max-w-2xl mx-auto">
        <div className="space-y-4">
          <Input
            ...
            placeholder="e.g., file system, git, database..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            onKeyPress={handleKeyPress}
            helperText="Search by functionality, technology, or server name"
          />
          ...
          <div className="flex items-center justify-between">
            ...
            {rateLimit && (
              <div className="text-sm text-gray-500">
                ...
                {rateLimit.isLimited && (
                  <span className="text-red-500 ml-2">(Rate limited)</span>
                )}
              </div>
            )}
          </div>
      ...
      {!isLoading && servers.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {servers.map((server) => (
              <Card key={server.id} hover className="cursor-pointer">
      ...
      {!isLoading && servers.length === 0 && !error && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-6">
            Try searching for specific functionality like "file system", "git", or "database"
          </p>
